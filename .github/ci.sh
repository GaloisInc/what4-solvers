...
build_yices() {
  if false ; then # "$IS_WIN"; then
    ...
    if $IS_WIN; then
      ...
      cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE=$TOP/scripts/libpoly-msys.cmake -DLIBPOLY_BUILD_PYTHON_API=Off -DCMAKE_INSTALL_PREFIX=$TOP/install-root -DHAVE_OPEN_MEMSTREAM=0
    else
